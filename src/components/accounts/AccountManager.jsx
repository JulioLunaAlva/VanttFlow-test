        ...
        <>
            <Dialog open={open} onOpenChange={(val) => {
                ...
                <DialogTrigger asChild>
                    <Button variant="ghost" className="h-12 px-6 rounded-2xl glass-premium border-white/10 hover:bg-white/10 text-white gap-3 group transition-all duration-500 hover:scale-105 active:scale-95 shadow-2xl">
                ...
                <DialogContent className="max-w-md glass-premium border-white/10 p-0 overflow-hidden rounded-[3rem] shadow-[0_40px_100px_rgba(0,0,0,0.8)]">
                    <div className="p-10 border-b border-white/5 bg-white/5 backdrop-blur-3xl relative overflow-hidden group">
                        ...
                        <DialogHeader className="relative z-10">
                            <DialogTitle className="text-3xl font-black tracking-tighter text-white drop-shadow-2xl">{editingAccount ? t('accounts.edit_account') || 'Editar Cuenta' : t('accounts.title') || 'Gestión de Cuentas'}</DialogTitle>
                            ...
                            <div className="space-y-4">
                                <div className="grid gap-3">
                                    {accounts.map(account => (
                                        <div key={account.id} className="flex items-center justify-between p-5 glass-premium border border-white/5 rounded-3xl hover:bg-white/5 transition-all duration-500 group/item relative overflow-hidden">
                                            <div className="flex items-center gap-4 relative z-10">
                                                <div className="w-10 h-10 rounded-2xl flex items-center justify-center bg-white/5 border border-white/10 shadow-inner group-hover/item:scale-110 transition-transform duration-500" style={{ color: account.color }}>
                                                    {getIcon(account.type)}
                                                </div>
                                                <div>
                                                    <p className="font-black text-sm tracking-tight text-white/90">{account.name}</p>
                                            ...
                                            <div className="flex gap-2 relative z-10">
                                                <Button variant="ghost" size="icon" className="h-9 w-9 bg-white/5 hover:bg-white/10 rounded-xl" onClick={() => handleEdit(account)}>
            ...
            <AlertDialog open={accountToDelete} onOpenChange={() => setAccountToDelete(null)}>
                <AlertDialogContent className="glass-premium border-white/10 rounded-[3rem] p-10 shadow-[0_40px_100px_rgba(0,0,0,0.8)]">
                    <AlertDialogHeader>
                        <AlertDialogTitle className="text-2xl font-black tracking-tight text-white mb-2">{t('accounts.confirm_delete') || '¿Estás seguro?'}</AlertDialogTitle>
                    ...
                    <AlertDialogFooter className="mt-8 gap-4">
                        <AlertDialogCancel className="bg-white/5 hover:bg-white/10 border-white/10 rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all">
